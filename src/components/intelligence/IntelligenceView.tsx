...
import { BrainCircuit, Sparkles, Megaphone, ShieldAlert, ArrowRight, Compass, Swords } from 'lucide-react';
...
export const IntelligenceView: React.FC = () => {
  ...
  if (jobs.length === 0 || !intelligence || !selectedJob) {
    return (
      <div className="p-12 text-center bg-white rounded-2xl border border-zinc-200 space-y-3">
        ...
        <h3 className="text-sm font-bold text-zinc-900">No Intelligence Generated Yet</h3>
        <p className="text-xs text-zinc-500 max-w-sm mx-auto">
          Run your first research job to synthesize competitive positioning gaps and audience signals.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      {/* Header with Job Selector & 2D Matrix Trigger */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-semibold shadow-2xs transition-colors"
          >
          ...
          {jobs.length > 1 && (
            <select
              ...
              className="p-2 rounded-xl border border-zinc-200 bg-white text-xs font-semibold text-zinc-800 outline-none shadow-2xs"
            >
              {jobs.map((j) => (
                <option key={j.id} value={j.id}>
                  {j.businessName} - {j.campaignObjective.slice(0, 25)}...
                </option>
              ))}
            </select>
          )}
